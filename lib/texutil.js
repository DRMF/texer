...
function generateSpecCode(cmd, parameterCount, replacement, argumentCount, renderOption, optionNum, code, extra) {
    ...
    for (var par = 0; par < parameterCount; par++) {
        if (pegjscode.indexOf("#" + (par + 1)) != -1) {
            temp = "l" + (par + 1);
            args.push(temp);
            pegjscode = pegjscode.replaceAll("{#" + (par + 1) + "}", "\" _ " + temp + ":lit _ \"").replaceAll("#" + (par + 1), "\" _ " + temp + ":lit _ \"");
        }
    }
    ...
    if (renderOption != null) {
        ...
        for (var arg = 0; arg < argumentCount; arg++) {
            temp = "k" + (arg + 1);
            args.push(temp);
            pegjscode = pegjscode.replaceAll("{#" + (arg + 1) + "}", "\" _ " + temp + ":lit _ \"").replaceAll("#" + (arg + 1), "\" _ " + temp + ":lit _ \"");
        }
    }
    pegjscode = (pegjscode + "\" _ ").replaceAll("_ \" ", "_ \"").replaceAll(" \" _", "\" _").replaceAll(" _ \"\" _ ", " ");
...
function generateGeneralCode(cmd, argumentCount, replacement, code) {
    ...
    var pegjscode = "\"\\" + cmd + "\"" + args.map(function(arg) {return " _ " + arg + ":lit"}).join("") +" ";
    pegjscode = [pegjscode].concat("{ return ast.Tex." + command + ["(\"\\" + cmd + "\""].concat(args).join(", ") + "); }");
    var astcode = command + ": { args: [ 'string', ";
...
//    rendercode += ["\"\\" + cmd + "\""].concat(args.map(function(a) {return "curlies(" + a + ")"})).join(" + ") + ";\n\t},";
    for (var i = 0; i < argumentCount; i++) {
        rendercode = rendercode.replaceAll("{#" + (i + 1) + "}", "\" + curlies(l" + (i + 1) + ") + \"").replaceAll(" + \"\"", "");
        rendercode = rendercode.replaceAll("#" + (i + 1), "\" + curlies(l" + (i + 1) + ") + \"").replaceAll(" + \"\"", "");
    }
    pegjs.push(pegjscode);
    ast.push(astcode);
...
module.exports.createMacroCode = function(commandDefinitions) {
    try {
        var code = [[], [], [], []];
        var fs = require('fs');
        var path = require('path');
        var sty = path.join(__dirname, "DRMFfcns.sty");
        ...
        if (typeof commandDefinitions != "undefined") {
            parseSty(commandDefinitions, code, module.exports.other_literals3);
        }
        module.exports.other_required = module.exports.other_literals3;
        pegjs = pegjs.concat(code[0]);
        ast = ast.concat(code[1]);
        astutil = astutil.concat(code[2]);
        render = render.concat(code[3]);
