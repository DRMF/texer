...
lit
  ...
  / f:generic_func &{ return tu.other_literals3[f]; } s:_ // from Texutil.find(...)
   {
     var ast = peg$parse(tu.other_literals3[f] + s);
  ...
  / BEGIN_ALIGN    opt_pos m:matrix END_ALIGN
    { return ast.Tex.MATRIX("align", lst2arr(m)); }
  / BEGIN_ALIGN_STAR    opt_pos m:matrix END_ALIGN_STAR
    { return ast.Tex.MATRIX("align*", lst2arr(m)); }
  ...
  / BEGIN_ALIGNAT  m:alignat END_ALIGNAT
    { return ast.Tex.MATRIX("alignat", lst2arr(m)); }
  ...
  / BEGIN_SPLIT m:matrix END_SPLIT
    { return ast.Tex.MATRIX("split", lst2arr(m)); }
...
litstuff =
     //basically lit without DELIMITER or LITERAL as an option (replaced by LITERALPART at the end), needed because they interrupt the parsing of commands in PAREN.
    f:PAREN atsymbol atsymbol l:lit   &{ return options.semanticlatex; }
    { return ast.Tex.PAREN2(f, l); }
  / f:PAREN atsymbol l:lit &{ return options.semanticlatex; }
    { return ast.Tex.PAREN1(f, l); }
...
LITERAL
 ...
 / f:generic_func &{ return tu.other_literals1[f]; } s:_ // from Texutil.find(...)
   { return ast.RenderT.TEX_ONLY(f + s); }
 / f:generic_func &{ return options.usemathrm && tu.other_literals2[f]; } s:_ // from Texutil.find(...)
   { return ast.RenderT.TEX_ONLY("\\mathrm{" + f + "}" + s); }
 / mathrm:generic_func &{ return options.usemathrm && mathrm === "\\mathrm"; } _
   "{" f:generic_func &{ return options.usemathrm && tu.other_literals2[f]; } _ "}" s:_
   /* make sure we can parse what we emit */
   { return options.usemathrm && ast.RenderT.TEX_ONLY("\\mathrm{" + f + "}" + s); }
 / f:generic_func &{ return tu.other_literals2[f]; } s:_ // from Texutil.find(...)
   { return ast.RenderT.TEX_ONLY("\\mbox{" + f + "}" + s); }
 / mbox:generic_func &{ return mbox === "\\mbox"; } _
   "{" f:generic_func &{ return tu.other_literals2[f]; } _ "}" s:_
 /* make sure we can parse what we emit */
  { return ast.RenderT.TEX_ONLY("\\mbox{" + f + "}" + s); }
 / f:(COLOR / DEFINECOLOR)
...
LITERALPART //LITERAL without the first option to prevent interruption of parsing
 ...
 / mbox:generic_func &{ return mbox === "\\mbox"; } _
   ...
   { return ast.RenderT.TEX_ONLY("\\mbox{" + f + "} "); }
 / f:generic_func &{ return tu.other_literals3[f]; } s:_ // from Texutil.find(...)
   { return ast.RenderT.TEX_ONLY(tu.other_literals3[f] + s); }
 / f:(COLOR / DEFINECOLOR)
   { return ast.RenderT.TEX_ONLY(f); }
 / "\\" c:[, ;!_#%$&] _
   { return ast.RenderT.TEX_ONLY("\\" + c); }
